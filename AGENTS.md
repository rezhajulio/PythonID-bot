...
- Circuit breaker (3 consecutive failures → 10 min cooldown; requests resume after the cooldown and a success closes the breaker) and a daily budget (default 15,000, reset at midnight WIB) stop a dead or rate-limited upstream from being hammered; both live in `classifier_client.py` and are covered by Hypothesis property tests
